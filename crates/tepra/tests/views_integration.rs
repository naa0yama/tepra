...
//!
//! `views::index` is currently a hardcoded stub (`printers: vec![]`). These
//! tests define the expected behaviour once the handler calls the Creator
//! API via `AppState.client`. All tests fail in the current state (RED).
#![allow(
    clippy::unwrap_used,
...
// ---------------------------------------------------------------------------
// 3. error_banner_contains_full_product_name
